...
struct VaultPreviewView: View {
    ...
    var body: some View {
        ...
        .task {
            // VaultItem isn't Sendable (it's a SwiftData @Model class) — capture
            // only the UUID into the detached task. VaultService's decrypt path
            // is nonisolated.
            let itemID = item.id
            let v = vault
    ...
    private func prepareShare() {
        ...
        // Re-encode to JPEG and write to a temp URL so iOS share targets get a file.
        guard let data = image.jpegData(compressionQuality: 0.92) else { return }
        let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("\(item.fileName).jpg")
